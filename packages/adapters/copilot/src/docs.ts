  ...
  notes: [
    {
      level: 'warn',
      message:
        'The MCP file’s top-level key is `servers`, not the `mcpServers` Claude Code and Cursor use. A `.mcp.json` copied to `.vscode/mcp.json` is valid JSON, loads without complaint and supplies no servers at all — a config that looks right and does nothing. Driftgate generates each from canonical rather than copying one to the other.',
      source: VSCODE_MCP_CONFIGURATION,
    },
    {
      level: 'info',
