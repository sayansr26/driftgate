...
`driftgate init` reads an existing **ruler** or **rulesync** setup and imports it into
`.driftgate/`. Nothing about it is one-directional in the other sense: Driftgate never
...
```
driftgate init          # prints everything it would do, writes nothing
driftgate init --yes    # applies it, backing up every file it takes ownership of
```
...
ruler and rulesync **generate the same files Driftgate's adapters import from** —
`AGENTS.md`, `CLAUDE.md`, `.cursor/rules/*`. A naive import would read both the source and
the copy built out of it, and write every rule into `.driftgate/` twice.
...
- **ruler is detected precisely.** Its output carries `<!-- Source: … -->` markers, so
  Driftgate can tell a file ruler wrote from one you wrote by hand. A `CLAUDE.md` with no
...
Driftgate imports **rules**. Anything else is reported by name and left alone:

- `ruler.toml` — including its MCP configuration. Driftgate has no TOML parser outside the
...
Skills and subagents are canonical models Driftgate has not built yet (T057–T060). When they
land, these importers gain them.

## After importing

Your `.ruler/` or `.rulesync/` directory is still there and still works; Driftgate has not
touched it. Once `driftgate check` reports the repository in sync, removing the old
