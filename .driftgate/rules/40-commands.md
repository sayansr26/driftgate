...
methods on it and never calls `applyPlan`. `--staged` is not implemented yet: reading the
git index means spawning `git`, and that arrives with the pre-commit hook package (T052).
