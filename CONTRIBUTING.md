...
outside contributor can close fastest, and `driftgate adapter new <tool>` exists to make it
...
- Each cell runs install ▸ **lint before build** (ESLint must resolve the workspace from
  ...
  the dist lane ▸ **`driftgate check` on this repository**.
...
- **Never write over, or delete, a file Driftgate did not generate.** `state.json` is the
  only record of ownership. `--force` may take ownership, but only after copying the
  original into `.driftgate/backup/`.
- **Adapters import `@driftgate/adapter-kit`, never `@driftgate/core`**, and never
...
`.driftgate/rules/`, run `driftgate sync`, and commit the rule and every regenerated file
together — CI runs `driftgate check` here, so a rule change without its artifacts fails the
build.
...
- `pnpm verify` is clean, and `DRIFTGATE_TEST_DIST=1 pnpm test` too if you touched
...
- **Bug** — include the command, the exit code, and `driftgate doctor --json`.
...
Driftgate is MIT licensed, and contributions are accepted under the same license.
