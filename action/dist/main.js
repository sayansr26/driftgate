...
var DRIFTGATE_DIR = ".driftgate";
var MANIFEST_PATH = `${DRIFTGATE_DIR}/driftgate.yaml`;
var RULES_DIR = `${DRIFTGATE_DIR}/rules`;
var RULES_GLOB = `${RULES_DIR}/**/*.md`;
var MCP_DIR = `${DRIFTGATE_DIR}/mcp`;
var MCP_SERVERS_PATH = `${MCP_DIR}/servers.yaml`;
var STATE_PATH = `${DRIFTGATE_DIR}/state.json`;
var BACKUP_DIR = `${DRIFTGATE_DIR}/backup`;
...
var MARKER_TEXT = "generated by driftgate; edit .driftgate/ instead";
...
var Validator = class {
  ...
  aliasTrap(node, field) {
    if (node === void 0 || !(0, import_yaml2.isAlias)(node))
      return false;
    this.errors.push(new DriftgateError({
      code: "E_YAML_SYNTAX",
...
function parseSecretMap(v, node, field, file) {
  ...
  for (const item of map.items) {
    ...
    if (ref === void 0) {
      v.errors.push(new DriftgateError({
...
async function parse(input) {
  ...
  if (manifestRaw !== void 0) {
    ...
    warnings.push(new DriftgateError({
...
async function computePlan(input) {
  ...
  for (const adapter of selected) {
    ...
    try {
      ...
      errors.push(cause instanceof DriftgateError ? cause : new DriftgateError({
...
var StagedFileSystem = class {
   ...
   */
  readFileRaw(relPath) {
    return Promise.reject(new DriftgateError({
      code: "E_GIT_NOT_STAGED",
...
var docs = {
  ...
  files: [
    {
      ...
      role: "instructions",
      managed: false,
      description: "Personal project-local overrides. Deprecated in favour of imports, and never generated by Driftgate \u2014 it is by definition not shared.",
    ...
    },
    {
      ...
      description: "Project memory, committed to the repository. The file Driftgate generates. Claude Code also reads CLAUDE.md files in subdirectories when it works on files beneath them.",
      source: CLAUDE_MEMORY_DOCS
    },
    {
      ...
      role: "instructions",
      managed: false,
      description: "User-level memory applied across every project. Read-only context for `doctor`: Driftgate never writes outside the repository.",
      source: CLAUDE_MEMORY_DOCS
    },
    {
  ...
  notes: [
    ...
    {
      level: "info",
      message: "Driftgate writes CLAUDE.md only. `.claude/settings.json` carries permissions and hooks \u2014 a materially different trust surface from instructions \u2014 and generating it is deliberately out of scope for v0."
    },
    {
      level: "warn",
  ...
  files: [
    {
      ...
      managed: false,
      nesting: "nearest-wins",
      description: "Checked before AGENTS.md at every level from the git root down. Never generated by Driftgate: its purpose is to override the shared file, so writing it would defeat it.",
    ...
    },
    {
      ...
      managed: true,
      nesting: "nearest-wins",
      description: "The file Driftgate generates. Codex walks from the git root to the working directory collecting one of these per level and concatenates them, so a nested AGENTS.md adds to the root file rather than replacing it \u2014 and, being later in the prompt, wins where they conflict.",
      source: CODEX_AGENTS_DOCS
    },
    {
      ...
      role: "instructions",
      managed: false,
      description: "User-level instructions applied to every project, read before any repository file. Read-only context for `doctor`: Driftgate never writes outside the repository.",
      source: CODEX_AGENTS_DOCS
    },
    {
...
var docs3 = {
  ...
  files: [
    {
      ...
      role: "instructions",
      managed: true,
      description: "Mechanism 1 of 3 \u2014 path-specific instructions. Applied only when the file being worked on matches the `applyTo` glob, and applied *in addition to* the repository-wide file, not instead of it. Driftgate generates one of these per glob-scoped canonical rule.",
    ...
    },
    {
      ...
      role: "instructions",
      managed: true,
      description: "Mechanism 2 of 3 \u2014 repository-wide instructions, always on for every request in the repository. Driftgate generates this from the canonical rules that carry no globs.",
      source: GITHUB_REPO_INSTRUCTIONS
    },
    {
      pattern: "AGENTS.md",
      ...
      managed: false,
      nesting: "nearest-wins",
      description: "Mechanism 3 of 3 \u2014 the cross-vendor agent file, also read by Copilot, with the nearest one in the directory tree taking precedence. Driftgate writes it from the **codex** adapter, never from this one: two adapters generating one path is an E_ARTIFACT_PATH_CONFLICT by design.",
      source: GITHUB_REPO_INSTRUCTIONS
    },
    {
      pattern: "CLAUDE.md",
    ...
    },
    {
      ...
      role: "instructions",
      managed: false,
      description: "User-level path-specific instructions, applied across projects and ranked above repository instructions. Read-only context for `doctor`: Driftgate never writes outside the repository.",
      source: VSCODE_CUSTOM_INSTRUCTIONS
    }
  ],
  limits: {
  ...
  notes: [
    ...
    {
      level: "info",
      message: "Driftgate writes instructions only. Prompt files (`.github/prompts/*.prompt.md`) and chat modes are a different surface \u2014 user-invoked rather than always-on \u2014 and generating them is deliberately out of scope for v0.",
      source: VSCODE_CUSTOM_INSTRUCTIONS
    }
  ]
};
  ...
  files: [
    {
      ...
      managed: false,
      nesting: "all-merged",
      description: "Component-level context. Gemini scans directories below the working directory, honouring .gitignore and .geminiignore, and appends what it finds. Driftgate writes only the root file; a nested one is somebody else\u2019s.",
    ...
    },
    {
      ...
      managed: true,
      nesting: "all-merged",
      description: "The file Driftgate generates. Gemini walks from the working directory up to the project root (the directory holding .git) collecting these, then concatenates them \u2014 a nested file adds to this one rather than replacing it.",
      source: GEMINI_CONTEXT_DOCS
    },
    {
      ...
      role: "instructions",
      managed: false,
      description: "User-wide context applied to every project, read before any repository file. Read-only context for `doctor`: Driftgate never writes outside the repository.",
      source: GEMINI_CONTEXT_DOCS
    },
    {
...
var HINT_SYNC = "hint: run: driftgate sync";
var HINT_HAND_EDITED = "hint: re-apply your edit in .driftgate/, then delete the generated file so sync can rewrite it.";
var HINT_ORPHAN_HAND_EDITED = "hint: delete the file yourself to accept the removal, or restore the rule that generated it in .driftgate/rules/";
var HINT_UNMANAGED = "hint: move the file aside to keep it, or run: driftgate sync --force (originals are copied to .driftgate/backup/ first)";
